...
function _economic_model_predictive_control_modeler_implementation(
    ...
    model_mlj::AutomationLabsIdentification.ResNet,
...
### Non linear eMPC implementation method with ResNet ###
...
### Mixed integer linear eMPC implementation method with ResNet ###
...
function _economic_model_predictive_control_modeler_implementation(
    ...
    for k = 1:1:horizon
        ...
        JuMP.@constraint(model_empc, x[:, k+1] .== W_layer[:][end] * y_af[:, end, k]) #output layer
    end
    ...
    for i = 1:1:horizon+1
        JuMP.@constraint(model_empc, e_x[:, i] .== x[:, i] - x_reference[:, i])
    end

    for i = 1:1:horizon
        JuMP.@constraint(model_empc, e_u[:, i] .== u[:, i] - u_reference[:, i])
    end
