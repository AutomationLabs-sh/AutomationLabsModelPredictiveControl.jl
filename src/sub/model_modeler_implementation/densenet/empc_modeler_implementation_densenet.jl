...
### Linear eMPC implementation method ###
...
### Non linear eMPC implementation method with DenseNet ###
function _economic_model_predictive_control_modeler_implementation(
    ...
    for i = 1:1:nbr_hidden+1
        symbol_tmp = Symbol("y_$i")
        model_empc[symbol_tmp] =
            JuMP.@variable(model_empc, [1:i*nbr_neurons, 1:nbr_hidden+1, 1:horizon])
    end

    f_activation = get_activation_function(model_mlj, system)
    JuMP.register(model_empc, :f_activation, 1, f_activation, autodiff = true)

    #Modeling the DenseNet into JuMP    
    for k = 1:1:horizon
        for i = 1:1:nbr_neurons
            JuMP.@constraint(
                model_empc,
                model_empc[:y_1][i, 1, k] == W_layer[1][i, :]' * [x[:, k]; u[:, k]]
            ) #input layer
        end
        ...
        for j = 2:1:nbr_hidden+1
            for i = 1:1:nbr_neurons
                JuMP.@NLconstraint(
                    model_empc,
                    model_empc[Symbol("y_$(j)")][i, j, k] == f_activation(
                        [
                            W_layer[j][i, :]' * model_empc[Symbol("y_$(j-1)")][:, j-1, k] +
                            B_layer[j-1][i],
                        ][1],
                    )
                ) #hidden layer
            ...
            JuMP.@constraint(
                model_empc,
                model_empc[Symbol("y_$(j)")][nbr_neurons+1:end, j, k] .==
                model_empc[Symbol("y_$(j-1)")][:, j-1, k]
            )
        end
        ...
        JuMP.@constraint(
            model_empc,
            x[:, k+1] .==
            W_layer[:][end] * model_empc[Symbol("y_$(nbr_hidden+1)")][:, end, k]
        ) #output layer
    end
...
### Mixed Integer linear eMPC implementation method with DenseNet ###
...
function _economic_model_predictive_control_modeler_implementation(
    ...
    for i = 1:1:nbr_hidden+1
        symbol_tmp = Symbol("y_$i") #output hidden neuron network after resnet addition
        model_empc[symbol_tmp] =
            JuMP.@variable(model_empc, [1:i*nbr_neurons, 1:nbr_hidden+1, 1:horizon])
    end
    ...
    #Modeling the densenet into JuMP with relu and MILP
    for k = 1:1:horizon
        for i = 1:1:nbr_neurons
            JuMP.@constraint(
                model_empc,
        ...
        for j = 1:1:nbr_hidden
            for i = 1:1:nbr_neurons
                ...
                JuMP.@constraint(
                    model_empc,
                    y_be[i, j, k] >= [
                        W_layer[j+1][i, :]' * model_empc[Symbol("y_$(j)")][:, j, k] +
                        B_layer[j][i],
                    ][1]
                )
                JuMP.@constraint(
                    ...
                    [
                        W_layer[j+1][i, :]' * model_empc[Symbol("y_$(j)")][:, j, k] +
                        B_layer[j][i],
                    ][1] .+ BigM * (1 .- Bin_1[i, j, k])
                )
                JuMP.@constraint(
                    model_empc,
                    y_be[i, j, k] .<= 0 .+ BigM * (1 .- Bin_2[i, j, k])
                )
                JuMP.@constraint(model_empc, Bin_1[i, j, k] + Bin_2[i, j, k] == 1)
            end
        end
        ...
        for j = 2:1:nbr_hidden+1
            JuMP.@constraint(
                model_empc,
                model_empc[Symbol("y_$(j)")][begin:nbr_neurons, j, k] .== y_be[:, j-1, k]
            )
            JuMP.@constraint(
                model_empc,
                model_empc[Symbol("y_$(j)")][nbr_neurons+1:end, j, k] .==
