...
### Linear MPC implementation method with Icnn ###
...
function _model_predictive_control_modeler_implementation(
    ...
    model_mlj::AutomationLabsIdentification.Icnn,
...
### Non linear MPC implementation method with Icnn ###
...
### Mixed integer linear MPC implementation method with Icnn ###
...
function _model_predictive_control_modeler_implementation(
    ...
    for k = 1:1:horizon
        ...
        for j = 2:1:nbr_hidden+1
            for i = 1:1:nbr_neurons
                JuMP.@constraint(model_mpc, y[i, j, k] >= 0)
                JuMP.@constraint(
                    model_mpc,
                ...
                JuMP.@constraint(model_mpc, Bin_1[i, j-1, k] + Bin_2[i, j-1, k] == 1)
            end
        end
