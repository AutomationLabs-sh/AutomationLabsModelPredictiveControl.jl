...
function _JuMP_model_definition( method::LinearProgramming, 
...
function _JuMP_model_definition( method::NonLinearProgramming, 
                                solver_selection::Union{ ipopt_solver_def, 
                                                         scip_solver_def} )
...
function _selection_solver_JuMP_model(solver::mosek_solver_def)
    ...
    model = JuMP.Model(JuMP.optimizer_with_attributes(MosekTools.Mosek.Optimizer,  "QUIET" => false,"OPTIMIZER_MAX_TIME" => 300.0, "INTPNT_CO_TOL_DFEAS" => 1e-7))
