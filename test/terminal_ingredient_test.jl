...
@testset "Contractive terminal ingredient computation" begin
    ...
    cons = JuMP.all_constraints(
        c_contractive.tuning.modeler;
        include_variable_in_set_constraints = false,
    )

    @test length(cons) == 75
    @test string(cons[end]) ==
          "-0.9 e_x[1,1]² - 0.9 e_x[2,1]² - 0.9 e_x[3,1]² - 0.9 e_x[4,1]² + e_x[1,6]² + e_x[2,6]² + e_x[3,6]² + e_x[4,6]² ≤ 0.0"
...
@testset "Neighborhood terminal ingredient computation" begin
    ...
    cons_neighborhood = JuMP.all_constraints(
        c_neighborhood.tuning.modeler;
        include_variable_in_set_constraints = false,
    )

    @test length(cons_neighborhood) == 95
    @test string(cons_neighborhood[75]) ==
          "1.1821257469929807 e_x[1,6] + 0.32477783075165184 e_x[2,6] + 3.753955653769598 e_x[3,6] - 0.6171907205612299 e_x[4,6] ≤ 1.2000000000000002"
    @test string(cons_neighborhood[76]) ==
          "1.9844516433175883 e_x[1,6] + 1.0527123209042824 e_x[2,6] + 4.957905573404276 e_x[3,6] - 0.15120229651054853 e_x[4,6] ≤ 1.2000000000000002"
    @test string(cons_neighborhood[77]) ==
          "-0.7174463800834686 e_x[1,6] - 0.16731968358133426 e_x[2,6] - 0.14299884029284093 e_x[3,6] + 0.5743028497226739 e_x[4,6] ≤ 0.45000000000000007"
    @test string(cons_neighborhood[78]) ==
          "1.6907988718505544 e_x[1,6] + 3.6956354224478427 e_x[2,6] + 0.8947871695749029 e_x[3,6] + 5.115699102348034 e_x[4,6] ≤ 1.2000000000000002"
    @test string(cons_neighborhood[79]) ==
          "3.2841587345393615 e_x[1,6] + 2.31606313179734 e_x[2,6] + 6.807979550060749 e_x[3,6] + 0.7214809704540696 e_x[4,6] ≤ 1.2000000000000002"
    @test string(cons_neighborhood[80]) ==
          "0.7917175253223642 e_x[1,6] + 0.18629471807030692 e_x[2,6] + 0.19261779399927226 e_x[3,6] - 0.5388689000604783 e_x[4,6] ≤ 0.7100000000000001"
    @test string(cons_neighborhood[81]) ==
          "-0.7917175253223642 e_x[1,6] - 0.18629471807030692 e_x[2,6] - 0.19261779399927226 e_x[3,6] + 0.5388689000604783 e_x[4,6] ≤ 0.45000000000000007"
    @test string(cons_neighborhood[82]) ==
          "0.1031718933941024 e_x[1,6] + 0.1083387497766164 e_x[2,6] - 0.7878420390865811 e_x[3,6] + 0.02814026958189298 e_x[4,6] ≤ 0.45000000000000007"
    @test string(cons_neighborhood[83]) ==
          "-5.355335429620435 e_x[1,6] - 4.56602538130204 e_x[2,6] - 9.82213786152917 e_x[3,6] - 2.0758851736694397 e_x[4,6] ≤ 2.0599999999999996"
    @test string(cons_neighborhood[84]) ==
          "3.426874514007263 e_x[1,6] + 5.553294145977776 e_x[2,6] + 2.5345823089219666 e_x[3,6] + 7.315365801939883 e_x[4,6] ≤ 1.2000000000000002"
    @test string(cons_neighborhood[85]) ==
          "5.355335429620435 e_x[1,6] + 4.56602538130204 e_x[2,6] + 9.82213786152917 e_x[3,6] + 2.0758851736694397 e_x[4,6] ≤ 1.2000000000000002"
    ...
    @test string(cons_neighborhood[92]) ==
          "-7.122630595237694 e_x[1,6] - 6.837395805570274 e_x[2,6] - 3.3426181833913065 e_x[3,6] - 14.372581083482705 e_x[4,6] ≤ 2.8"
    @test string(cons_neighborhood[93]) ==
          "-8.103867109868654 e_x[1,6] - 9.72924467090224 e_x[2,6] - 16.263131538649258 e_x[3,6] - 1.5721879837156578 e_x[4,6] ≤ 2.0599999999999996"
    @test string(cons_neighborhood[94]) ==
          "7.122630595237694 e_x[1,6] + 6.837395805570274 e_x[2,6] + 3.3426181833913065 e_x[3,6] + 14.372581083482705 e_x[4,6] ≤ 1.2000000000000002"
    @test string(cons_neighborhood[95]) ==
          "8.103867109868654 e_x[1,6] + 9.72924467090224 e_x[2,6] + 16.263131538649258 e_x[3,6] + 1.5721879837156578 e_x[4,6] ≤ 1.2000000000000002"
