...
### Non linear MPC implementation method with DenseNet ###
...
function _model_predictive_control_modeler_implementation(
    ...
    for i = 1:1:nbr_hidden+1
        ...
        model_mpc[symbol_tmp] =
            JuMP.@variable(model_mpc, [1:i*nbr_neurons, 1:nbr_hidden+1, 1:horizon])
    end

    f_activation = get_activation_function(model_mlj, system)
    JuMP.register(model_mpc, :f_activation, 1, f_activation, autodiff = true)

    #Modeling the Fnn into JuMP    
    for k = 1:1:horizon
        for i = 1:1:nbr_neurons
            JuMP.@constraint(
                model_mpc,
                model_mpc[:y_1][i, 1, k] == W_layer[1][i, :]' * [x[:, k]; u[:, k]]
            ) #input layer
        end
        ...
        for j = 2:1:nbr_hidden+1
            for i = 1:1:nbr_neurons
                JuMP.@NLconstraint(
                    model_mpc,
                    model_mpc[Symbol("y_$(j)")][i, j, k] == f_activation(
                        [
                            W_layer[j][i, :]' * model_mpc[Symbol("y_$(j-1)")][:, j-1, k] +
                            B_layer[j-1][i],
                        ][1],
                    )
                ) #hidden layer
            ...
            JuMP.@constraint(
                model_mpc,
                model_mpc[Symbol("y_$(j)")][nbr_neurons+1:end, j, k] .==
                model_mpc[Symbol("y_$(j-1)")][:, j-1, k]
            )
        end
        ...
        JuMP.@constraint(
            model_mpc,
            x[:, k+1] .==
            W_layer[:][end] * model_mpc[Symbol("y_$(nbr_hidden+1)")][:, end, k]
        ) #output layer
    end
...
### Mixed Integer linear MPC implementation method with DenseNet ###
...
function _model_predictive_control_modeler_implementation(
    ...
    for k = 1:1:horizon
        ...
        for j = 1:1:nbr_hidden
            for i = 1:1:nbr_neurons
                ...
                JuMP.@constraint(
                    model_mpc,
                    y_be[i, j, k] >= [
                        W_layer[j+1][i, :]' * model_mpc[Symbol("y_$(j)")][:, j, k] +
                        B_layer[j][i],
                    ][1]
                )
                JuMP.@constraint(
        ...
        for j = 2:1:nbr_hidden+1
            JuMP.@constraint(
                model_mpc,
                model_mpc[Symbol("y_$(j)")][begin:nbr_neurons, j, k] .== y_be[:, j-1, k]
            )
            JuMP.@constraint(
                model_mpc,
                model_mpc[Symbol("y_$(j)")][nbr_neurons+1:end, j, k] .==
